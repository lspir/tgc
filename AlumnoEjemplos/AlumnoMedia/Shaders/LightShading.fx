...
float3 fvLightPosition1 = float3( 0, 0, 0 );
float3 fvLightPosition2 = float3( 0, 10, 0 );
...
{      
	...
	float3 LD = normalize(fvLightPosition1-float3(Pos.x,Pos.y,Pos.z));
	float3 LD2 = normalize(fvLightPosition2-float3(Pos.x,Pos.y,Pos.z));
	ld += saturate(dot(N, LD))*k_ld;
	ld += saturate(dot(N, LD2))*k_ld;
	...
	float ks2 = saturate(dot(reflect(LD2,N), D));
	ks2 = pow(ks2,fSpecularPower);
	le += ks*k_ls;
	le += ks2*k_ls;
